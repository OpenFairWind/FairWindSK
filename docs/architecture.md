...
- **Application registry (`AppItem`)**: Represents embedded web applications and typed Android launcher activities together with their metadata, activation state, and ordering. Web items are refreshed from the Signal K server’s Apps API (`/signalk/v1/apps/list`) and merged with local overrides. Android 13/API 33 and newer builds add operator-selected native activities through `fairwind.source=android`; native `file://` applications remain blocked.
...
3. **Application discovery**: `FairWindSK::loadApps()` requests `/signalk/v1/apps/list` from the configured server and falls back to the legacy `/skServer/webapps` path if needed. Each returned web app becomes an `AppItem`. On Android, `FairWindSKActivity` queries only `MAIN + LAUNCHER` activities; operator-selected results are persisted as typed `AppItem` records and merged into the same registry. Local apps preserve order and activation, while inactive or missing server apps are demoted but retained.
...
- Top Bar and Bottom Bar data readouts now use reusable `DataWidget` instances backed by `dataWidgets` configuration entries. Each instance owns its own Signal K subscription lifecycle and initial REST snapshot hydration while sharing the same live/stale/missing trust grammar.
