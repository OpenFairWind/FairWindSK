# FairWindSK documentation

The English, French, Spanish, and Italian LaTeX owner manuals and their reproducible PDF build instructions
are documented in [manual/README.md](manual/README.md).

- [Getting started](./getting_started.md)
- [Hardware requirements](./hardware.md)
- [Building FairWindSK](./building.md)
- [Building FairWindSK on macOS](./macos.md)
- [Building FairWindSK on Windows](./windows.md)
- [Building and running FairWindSK on Linux](./linux.md)
- [Building and running FairWindSK on Raspberry Pi OS](./raspberrypi.md)
- [Building and running FairWindSK in a container](./container.md)
- [Android build, signing, and launcher guide](./android.md)
- [iOS and iPadOS build and simulator guide](./ios.md)
- [Developing guide](./developing_guide.md)
- [Automated testing](./testing.md)
- [Versioning and releases](./releasing.md)
- [App Store and Google Play publishing package](./stores/README.md)
- [FairWindSK user guide](./fairwindsk_user_guide.md)
- [Architecture overview](./architecture.md)
- [UI shell definition](./ui_shell.md)
- [Configuration guide](./configuring.md)
- [Running a companion Signal K Server with Docker](./signalk-server-docker.md)
- [Multilingual support](./multilingual.md)
- [GUI overview](./gui.md)
- [Typical use cases](./use_case.md)
