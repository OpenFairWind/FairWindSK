...
Each bar-layout entry stores the item kind (`widget`, `separator`, or `stretch`), the widget id when applicable, ordering, width/height expansion, and display toggles for icon, text, units, and trend. Signal K data widgets can be shown on both the Top Bar and Bottom Bar at the same time; each visible instance subscribes independently. Fixed shell widgets are offered only on the bar where they can actually render, so the Top Bar editor exposes the current-context and clock/status block while the Bottom Bar editor exposes the launcher controls, open-app strip, and Signal K server box.
