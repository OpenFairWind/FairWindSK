...
- Keep Android package discovery scoped to launchable activities through `MAIN + LAUNCHER` queries. Do not add `QUERY_ALL_PACKAGES` unless a reviewed product requirement and distribution-policy analysis explicitly justify it.
- Keep the Android Home role optional: installation must never select FairWindSK as the default Home app automatically, and the ordinary application-launcher intent must remain available.
- Native Android applications selected under **Settings > Android** belong to the shared application palette and launcher-page model. Preserve their `fairwind.source=android`, package, activity, stable identity, icon behavior, deselection cleanup, and explicit-intent launch path.
- For Android launcher changes, verify on API 33 and on the current target API: package enumeration, icon rendering, app selection persistence, disabled incompatible Applications actions, explicit activity launching, Back/Home return behavior, and recovery to another Home app.
- Keep Android selector launch icons, names, and `TouchCheckBox` controls in separate non-overlapping touch regions. When FairWindSK is Home, expose soft Back/Home/Recents only for hardware keys Android reports absent; keep Recents inside FairWindSK and limit it to native activities launched through FairWindSK.
...
- **Android launcher integration**: available only in Android builds on the Android 13/API 33 minimum runtime. FairWindSK remains a normal app and may optionally be selected as the system Home app. **Settings > Android**, placed after System, selects which installed `MAIN + LAUNCHER` activities join the shared Applications palette. Selected native apps can be assigned alongside Signal K apps to existing launcher pages. Their identity and metadata are Android-managed, so Applications disables incompatible edit/remove/web actions while preserving coherent page-placement actions. Deselecting an Android app removes its launcher-page references.
- **Android build documentation**: [docs/android.md](./docs/android.md) is the authoritative Linux, Windows, and macOS environment, signed-APK, deployment, launcher, and troubleshooting guide. Keep shorter Android sections elsewhere linked to it and consistent with its API 33 contract.
