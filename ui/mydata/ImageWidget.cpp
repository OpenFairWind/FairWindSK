...
void ImageWidget::setPixmap(const QPixmap &pixmap, const int desiredWidth,
...
QPixmap ImageWidget::pixmap() const { return m_item.pixmap(); }

void ImageWidget::scale(qreal s) { QGraphicsView::scale(s, s); }

void ImageWidget::resize(int desiredWidth, int desiredHeight) {
...
void ImageWidget::zoomIn() { scale(1.2); }
...
void ImageWidget::wheelEvent(QWheelEvent *event) {
  ...
  if (event->source() == Qt::MouseEventNotSynthesized) {
    if (event->angleDelta().y() > 0) {
      scale(ImageWidget::ZOOM_IN_SCALE);
    } else {
      scale(ImageWidget::ZOOM_OUT_SCALE);
    }
  }
  QGraphicsView::wheelEvent(event);
}

bool ImageWidget::nativeGestureEvent(QNativeGestureEvent *event) {
